-- Make shadow re-embedding safely parallel and crash recoverable. A candidate
-- must be claimed by one lease owner before any provider work; only that owner
-- may commit the vector or the bounded retry record.

ALTER TABLE chunk_embedding_vectors
    DROP CONSTRAINT chunk_embedding_vectors_status_check,
    DROP CONSTRAINT chunk_embedding_vectors_state_check,
    ADD COLUMN lease_owner UUID,
    ADD COLUMN leased_at TIMESTAMPTZ,
    ADD COLUMN available_at TIMESTAMPTZ NOT NULL DEFAULT NOW();

ALTER TABLE chunk_embedding_vectors
    ADD CONSTRAINT chunk_embedding_vectors_status_check CHECK (
        status IN ('pending', 'processing', 'embedded', 'failed')
    ),
    ADD CONSTRAINT chunk_embedding_vectors_state_check CHECK (
        (
            status = 'pending'
...
DROP INDEX idx_chunk_embedding_vectors_pending;

CREATE INDEX idx_chunk_embedding_vectors_claim
    ON chunk_embedding_vectors(generation_id, status, available_at, chunk_id)
    WHERE status IN ('pending', 'failed');

CREATE INDEX idx_chunk_embedding_vectors_expired_lease
    ON chunk_embedding_vectors(generation_id, leased_at, chunk_id)
    WHERE status = 'processing';

CREATE FUNCTION claim_chunk_embedding_vectors(
...
DROP FUNCTION record_chunk_embedding_vector(UUID, UUID, vector, JSONB);
...
DROP FUNCTION record_chunk_embedding_failure(UUID, UUID, VARCHAR, TEXT);
...
COMMENT ON FUNCTION claim_chunk_embedding_vectors(UUID, UUID, INT, INT) IS
    'Atomically claims a bounded SKIP LOCKED batch and recovers expired leases before any provider calls occur.';
COMMENT ON COLUMN chunk_embedding_vectors.lease_owner IS
    'Per-attempt ownership fence; a late provider response cannot overwrite a newer worker attempt.';
